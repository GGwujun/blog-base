...
function isCode(node) {
  ...
  var isCodeBlock = node.parentNode && node.parentNode.nodeName === 'PRE' && !hasSiblings;
  return node.nodeName === 'CODE' && !isCodeBlock;
}

function isKeep(options, node) {
  var filters = options.keepFilter ? options.keepFilter : ['div', 'style'];
  return Array.isArray(filters) ? filters.some(function (filter) {
    return filter === node.nodeName.toLowerCase();
  }) : typeof filters === 'function' ? filters(node, options) : filters === node.nodeName.toLowerCase();
}

function fenceReplacement(content, node, options) {
  var className = node.firstChild ? node.firstChild.className : '';
  var language = (className.match(/language-(\S+)/) || [null, ''])[1];
  var startFence = options.startFence != undefined ? options.startFence : options.fence;
  var endFence = options.endFence != undefined ? options.endFence : options.fence;
  var parent = node.parentNode;
  var parentIsList = parent && parent.nodeName === 'LI';
  return (parentIsList ? '\n' : '\n\n') + startFence + language + '\n' + (node.firstChild ? node.firstChild.textContent : '') + (node.firstChild && node.firstChild.textContent && node.firstChild.textContent.endsWith('\n') || !content ? '' : '\n') + endFence + '\n\n';
}
...
function IndentCodeIsListfirstChild(list, options) {
  return options.codeBlockStyle !== 'fenced' && list && list.firstChild && list.nodeName === 'LI' && list.firstChild.nodeName === 'PRE';
}
...
function () {
  ...
  _proto.caclPrefix = function caclPrefix(input) {
    ...
    if (parent && parentIsOL) {
      ...
      prefix = (start ? Number(start) + index : index + 1) + (isNewList ? ')  ' : '.  ');
    }
  ...
  _createClass(ListNode, [{
    ...
    get: function get() {
      ...
      return parent && parent.previousSibling && parent.previousSibling.nodeName === parent.nodeName;
    }
  }, {
    key: "followCode",
    get: function get() {
      var parent = this.parent;
      return parent && parent.nextSibling && parent.nextSibling.nodeName === 'PRE';
    }
  }, {
    key: "inLast",
    get: function get() {
    ...
    get: function get() {
      var nestOLCount = this.nestOLCount,
          nestULCount = this.nestULCount,
          node = this.node;
      return nestOLCount + nestULCount > 1 && node.parentNode && node.parentNode.parentNode && node.parentNode.parentNode.innerHTML === node.parentNode.outerHTML;
    }
  }]);
...
function blankReplacement(content, node, options) {
  if (isKeep(options, node)) {
    ...
    return delimiter + (options.codeBlockStyle === 'fenced' ? ' ' : '') + (content || ' ') + delimiter + '\n';
  ...
  service.addRule('heading', {
    ...
    replacement: function replacement(content, node, options) {
      ...
      if (options.headingStyle === 'setext' && hLevel < 3) {
        ...
        return '\n\n' + repeat('#', hLevel) + ' ' + escape(escapes$1, content) + '\n\n';
      }
    }
  });
};
...
var applyReferenceLinkRule = function applyReferenceLinkRule(service) {
  service.addRule('referenceLink', {
    ...
    replacement: function replacement(content, node, options) {
      var href = escape(escapes$2, decodeURIComponent(node.getAttribute('href')));
      ...
      switch (options.linkReferenceStyle) {
        ...
        default:
          var id = this.references && this.references.length ? 'ref' + (this.references.length + 1) : 'ref';
...
var escapes$3 = [[/(.*)\|(.*)/g, function (match, p1, p2) {
  if (match.match(/\`.*\|.*\`/)) {
    return p1 + "|" + p2;
  }
...
function cell(content, node, options) {
  var index = node.parentNode ? indexOf.call(node.parentNode.childNodes, node) : -1;
  var prefix = ' ';
...
function isHeadingRow(tr) {
  ...
  return parentNode ? parentNode.nodeName === 'THEAD' || parentNode.firstChild === tr && (parentNode.nodeName === 'TABLE' || isFirstTbody(parentNode)) && every.call(tr.childNodes, function (n) {
...
var applyTableRule = function applyTableRule(service) {
    ...
    replacement: function replacement(content, node) {
      ...
      if (isHeadingRow(node)) {
        for (var i = 0; i < node.childNodes.length; i++) {
          ...
          var align = (curNode.getAttribute('align') || curNode.style.textAlign || '').toLowerCase();
  ...
  service.addRule('noHeaderTable', {
    filter: function filter(node, options) {
      ...
      if (node.nodeName === 'TABLE' && !hasHead && options.convertNoHeaderTable) {
        try {
...
function caclListIndent(node, options) {
  ...
  if (nestOLCount) {
    ...
    var isFirstChild = parentNode && parentNode.firstChild && parentNode.nodeName === 'LI' && parentNode.firstChild === node;
    var IndentCodeIsfirstChild = IndentCodeIsListfirstChild(parentNode, options);
    return nestULCount * 2 + nestOLCount * 4 + 4 + findOrderListIndentNumber(node) + (isFirstChild ? -4 : 0) + (IndentCodeIsfirstChild ? -1 : 0);
  }
...
var applyEmRule = function applyEmRule(service) {
  service.addRule('em', {
    ...
    replacement: function replacement(content, node, options) {
      ...
      if (node.parentNode && node.parentNode.nodeName === 'EM' && node.parentNode.firstChild === node.parentNode.lastChild) {
...
var applyLinkRule = function applyLinkRule(service) {
  service.addRule('link', {
    ...
    replacement: function replacement(content, node) {
      ...
      var normalizeHref = href ? decodeURIComponent(href).replace('mailto:', '') : '';

      if (node.firstChild && normalizeHref === node.firstChild.nodeValue) {
        return '<' + node.firstChild.nodeValue + '>';
      }

      if (href && normalizeHref.split('').some(function (_char) {
...
var applyTaskRule = function applyTaskRule(service) {
  service.addRule('task', {
    filter: function filter(node) {
      return node.type === 'checkbox' && node.parentNode != null && node.parentNode.nodeName === 'LI';
    },
    replacement: function replacement(_content, node) {
...
var applyPlugins = (function (service) {
  service.use([applyListRule, applyHrRule, applyParagraphRule, applyHeadingRule, applyFenceRule, applyCodeRule, applyReferenceLinkRule, applyTableRule, applyIndentedCodeBlockRule, applyBlockquoteRule, applyEmRule, applyDelRule, applyLinkRule, applyBrRule, applyStrongRule, applyImageRule, applyTaskRule]);
  service.keep(function (node) {
    if (isKeep(service.options, node)) {
      if (node.parentNode) {
        var index = Array.from(node.parentNode.childNodes).findIndex(function (n) {
          return n === node;
        });
...
function collapseWhitespace(options) {
  ...
  while (node !== element) {
    if (node.nodeType === 3 || node.nodeType === 4) {
      ...
      if ((!prevText || !prevText.data || / $/.test(prevText.data)) && !prevVoid && text[0] === ' ') {
        text = text.substr(1);
...
var HTMLParser = /*#__PURE__*/
canParseHTMLNatively() ? root.DOMParser : Parser;
...
function createRootNode(input) {
  ...
  if (typeof input === 'string') {
    ...
    // a single element.
    '<x-sitdown id="root-node">' + input + '</x-sitdown>', 'text/html');
...
function isBlank(node) {
  return ['A', 'TABLE', 'THEAD', 'TBODY', 'TR', 'TH', 'TD', 'IFRAME', 'SCRIPT', 'AUDIO', 'VIDEO'].indexOf(node.nodeName) === -1 && /^\s*$/i.test(node.textContent || '') && !isVoid(node) && !hasVoid(node);
}
...
var Node = function Node(node) {
  ...
  newNode.isCode = node.nodeName.toLowerCase() === 'code' || node.parentNode.isCode;
...
function separatingNewlines(output, replacement) {
  ...
  var newlines = [outputNewLines ? outputNewLines[0] : '', replacementNewLines ? replacementNewLines[0] : ''].sort();
...
var escapes$5 = [[/\\/g, '\\\\'], [/\*/g, '\\*'], [/^-/g, '\\-'], [/^\+ /g, '\\+ '], [/^(=+)/g, '\\$1'], [/^(#{1,6}) /g, '\\$1 '], [/`/g, '\\`'], [/^~~~/g, '\\~~~'], [/\[/g, '\\['], [/\]/g, '\\]'], [/^>/g, '\\>'], [/_/g, '\\_'], [/^(\d+)\. /g, '$1\\. '], // escapes
[/^(\d+)\. /g, '$1\\. '], [/!/g, '\\!'], // [/"/g, '\\"'],
// [/'/g, `\\'`],
// [/#/g, '\\#'],
[/\$/g, '\\$'], [/%/g, '\\%'], [/&/g, '\\&'], [/\(/g, '\\('], [/\)/g, '\\)'], // [/\+/g, '\\+'],
...
function () {
  ...
  _proto.process = function process(parentNode) {
    ...
    var rst = reduce.call(parentNode.childNodes, function (output, node) {
      node = new Node(node);
      var replacement = '';

      if (node.nodeType === 3) {
        replacement = node.isCode || node.unNeedEscape ? node.nodeValue : _this.escape(node.nodeValue);
      } else if (node.nodeType === 1) {
  ...
  _proto.replacementForNode = function replacementForNode(node) {
    ...
    return whitespace.leading + (rule.replacement ? rule.replacement(content, node, this.options) : '') + whitespace.trailing;
  };
...
function () {
  function Sitdown(options) {
    ...
    this.service = new Service(_extends({}, this.defaultOptions, {}, options));
...
export { RootNode, Sitdown, index as Util };
